...
app.get("/players/:playerId/", async (request, response) => {
  ...
  const getPlayerQuerys = `SELECT * FROM player_details 
    WHERE
    player_id=${playerId};`;
  const player = await database.get(getPlayerQuerys);
...
app.get("/matches/:matchId/players", async (request, response) => {
  ...
  const getMatchPlayersQuery = `
    ...
    FROM
        ...
        WHERE
        match_id=${matchId};`;
  const playerArrays = await database.all(getMatchPlayersQuery);
  response.send(
    playerArrays.map((eachPlayer) =>
      convertPlayerDbObjectToResponseObject(eachPlayer)
    )
  );
});
